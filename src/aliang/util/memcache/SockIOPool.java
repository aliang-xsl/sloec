...
public class SockIOPool {
	...
	public void initialize() {
		...
		synchronized (this) {
			...
			if (initialized && (buckets != null) && (availPool != null)
					&& (busyPool != null)) {
			...
			availPool = new HashMap<String, Map<SockIO, Long>>(servers.length
					* initConn);
			busyPool = new HashMap<String, Map<SockIO, Long>>(servers.length
					* initConn);
			...
			maxCreate = (poolMultiplier > minConn) ? minConn : minConn
					/ poolMultiplier; // only create up to maxCreate connections
										// at once
	...
	protected SockIO createSocket(String host) {
		...
		try {
			...
			if (!socket.isConnected()) {
				...
				try {
					...
					log.error("++++ failed to close SockIO obj for server: "
							+ host);
		...
		try {
			if (socket == null) {
				...
				log.debug("++++ ignoring dead host: " + host + " for " + expire
						+ " ms");
	...
	protected void selfMaint() {
		...
		synchronized (this) {
			...
			for (Iterator<String> i = availPool.keySet().iterator(); i
					.hasNext();) {
				String host = i.next();
